...
from ecg_visualization.scripts.ltafdb_concat.constants import (
...
AF_RHYTHM_PREFIX = "(AFIB"
SINUS_RHYTHM_PREFIX = "(N"
RHYTHM_AF = "af"
RHYTHM_SINUS = "sinus"
...
def iter_concatenated_sequences() -> Iterable[tuple[ECGEntity, ConcatenatedSequence]]:
    ...
    for entity in dataset.data_entities:
        ...
        if segments_info is None:
            LOGGER.info("Skipping %s: no AF/sinus segments found.", entity.entity_id)
            continue
...
def _select_segments_info(entity: ECGEntity) -> SegmentsInfo | None:
    ...
    af_intervals = [
        ...
        if interval.rhythm == RHYTHM_AF
        and interval.end_sec - interval.start_sec >= SEGMENT_DURATION_SEC
        and interval.start_sec >= SEGMENT_DURATION_SEC
    ]

    for af_interval in sorted(af_intervals, key=lambda interval: interval.start_sec):
        pre_af = SegmentWindow(
            af_interval.start_sec - SEGMENT_DURATION_SEC,
            af_interval.start_sec,
        )
        af = SegmentWindow(
            af_interval.start_sec,
            af_interval.start_sec + SEGMENT_DURATION_SEC,
        )
        sinus_windows = _select_sinus_windows(rhythm_intervals, pre_af, af)
    ...
    LOGGER.warning("Skipping %s: no eligible AF event found.", entity.entity_id)
    return None
...
def _rhythm_from_aux_note(aux_note: str) -> str | None:
    note = aux_note.strip()
    if note.startswith(AF_RHYTHM_PREFIX):
        return RHYTHM_AF
    if note.startswith(SINUS_RHYTHM_PREFIX):
        return RHYTHM_SINUS
    return None
...
def _select_sinus_windows(
    ...
    excluded = SegmentWindow(pre_af.start_sec, af.end_sec)
